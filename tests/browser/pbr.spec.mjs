...
test.beforeAll(async () => {
  const { outputFiles } = await build({ entryPoints: [resolve('_build/js/release/build/examples/pbr/pbr.js')], bundle: true, platform: 'browser', format: 'esm', write: false });
...
test('HDR URL loading and PMREM illuminate a material without lights', async ({ page }) => {
  ...
  const hdr = await readFile(new URL('../fixtures/studio.hdr', import.meta.url));
...
test('loaded PhysicalMaterial switches shader paths when transmission changes', async ({ page }) => {
  ...
  const glb = await readFile(new URL('../fixtures/physical.glb', import.meta.url));
