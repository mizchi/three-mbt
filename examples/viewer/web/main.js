...
const characters = {
  rabbit: {
    title: 'Moon rabbit', label: 'Interactive low-poly MoonBit rabbit model', number: '02 / MOON',
  ...
  mouse: {
    title: 'Little mouse', label: 'Interactive low-poly mouse model', number: '01 / MUS',
...
let selected = new URLSearchParams(location.search).get('model') === 'mouse' ? 'mouse' : 'rabbit';
let viewer;
let animation;
let observer;
let stopped = false;
...
function updateCharacter() {
  ...
  document.querySelector('#study-number').textContent = `MODELING STUDY / ${selected === 'rabbit' ? '002' : '001'}`;
...
try {
  ...
  if (selected === 'mouse') await viewerApi.select_model(viewer, selected);
